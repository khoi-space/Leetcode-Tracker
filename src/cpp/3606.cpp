...
vector<string> validateCoupons(vector<string>& code, vector<string>& businessLine, vector<bool>& isActive) {
    #if APR == 1
    // Hashmap<string, int> (string for businessLines, int for check valid
    ...
    return valid_codes;
    #elif APR == 2
    // Hashmap<string, vector<string>> (string for businessLines, vector<string> for their valid codes)
    int n = max(code.size(), max(businessLine.size(), isActive.size()));
    vector<string> valid_codes;
